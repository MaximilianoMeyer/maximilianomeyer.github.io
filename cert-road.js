(() => {
  ...
  let running=false, raf=0, last=0, roadTime=0, carX=0, speed=48, passed=0, active=-1;
  ...
  function reset(){roadTime=0;carX=0;speed=48;passed=0;active=-1;signs.forEach((s,i)=>{s.z=1400+i*1050;s.shown=false;});card.classList.remove('show');updateHud();}
  ...
  function update(dt){
    const target=keys.boost?78:48;speed+=(target-speed)*Math.min(1,dt*2.4);
