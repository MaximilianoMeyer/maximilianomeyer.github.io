(() => {
  ...
  function reset(){roadTime=0;carX=0;speed=88;passed=0;active=-1;signs.forEach((s,i)=>{s.z=1400+i*1050;s.shown=false;});card.classList.remove('show');updateHud();}
  ...
  function update(dt){
    const target=keys.boost?148:88;speed+=(target-speed)*Math.min(1,dt*3.2);
  ...
  window.openCertRoad=function(){
    reset();modal.classList.add('open');modal.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';running=true;last=performance.now();startupSound();canvas.focus();raf=requestAnimationFrame(loop);
  };
  function close(){running=false;cancelAnimationFrame(raf);modal.classList.remove('open');modal.setAttribute('aria-hidden','true');document.body.style.overflow='';card.classList.remove('show');}
